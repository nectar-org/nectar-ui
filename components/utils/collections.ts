...
export const isNectarElement = (el?: HTMLElement): boolean => {
  if (!el) return false
  if (el?.dataset && el?.dataset['nectar']) return true
  el.attributes.getNamedItem('data-nectar')
  return !!el.attributes.getNamedItem('data-nectar')
}
