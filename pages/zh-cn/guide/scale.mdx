...
<Note type="warning">
  此功能需要 Nectar v2.2.0 或以上版本。如果你正在使用旧版本，请在试用前升级到最新。
</Note>

Nectar UI 的可伸缩特性赋予了每个组件极大的灵活性。你可以自由地缩放组件大小，改写组件局部空间的比例，调整内、外的布局结构等。
...
与常见的组件库不同的是，Nectar UI 不再使用 `size` 相关属性控制组件大小。每个组件有默认合适的布局体积，
你可以通过 `scale` 属性动态地缩放组件。

如下所示，`Button` 组件的所有空间占比 (包含字体) 均等比缩小。Nectar UI 会通过动态计算组件大小，**真实地渲染指定体积组件**，
这确保了布局与排版始终与预期相同。
...
组件的空间体积同时取决于用户输入的 `scale` 属性与**缩放单位**，默认的缩放单位为 `16px`，但 Nectar UI 允许你在不同层级上自定义此值以达到自定义级别的动态效果。
...
`NectarProvider` 作用域，在此作用域下的所有组件都会共享同一个*缩放单位*。(请参考 [主题小节](/zh-cn/guide/themes '主题') 了解自定义主题)
...
Nectar 组件库中的 Scale 功能也对外导出，你可以使用此工具创建具备可伸缩功能的组件。
