...
之所以要在 _Nectar_ 内单独创建一个 Hooks 解决类名合并的问题而不是直接引用社区第三方库，很大一部分考量是来自于体积。
_Nectar_ 组件库内不需要复杂的类名合并、处理多样性的表达式，更多仅用于移除空白字符。如果你正在 Css in JS 的样式解决方案，这会是一个对你非常有帮助的轻量级类名合并工具。
...
```tsx
import { useClasses } from '@nectar-ui/core'
